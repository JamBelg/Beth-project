---
title: "CAS Thesis in Advanced Machine Learning"
subtitle: |
...
# Data analysis
The Beth dataset represents more than 8 millions events collected over 23 honeypots, only nearly 1 million of it will be used on this project.
Data are already divided into training, validating and testing dataset (60% / 20% /20%).

## Features
Each of this dataset (training, validating and testing dataset) has features showed in the below table. We have also created charts that provide visual insight into the frequency distribution of various entities (like host names, process names, threadId, etc.) using logarithmic scaling and color gradients to highlight differences in counts.

 ![Features](pics/features.png){width=30%, height=20%}

 - timestamp: time in seconds since system  boot (float)

 - processId: id of the process spawning this log (integer)\
  The bar chart shows the top 20 process IDs (PIDs) based on their count. The process labeled “Else” has the highest count, far exceeding the counts of any individual Process ID. PID 159 has the second-highest count, noticeably higher than the rest of the process IDs, though much lower than “Else”. Each colored bar represents a distinct process ID, with colors transitioning from dark purple for higher counts to lighter green for lower counts.
 
 ![Process ID](pics/train_processid_plot.png){width=30%, height=30%}


 - threadId: id of the thread (integer) \
 The thread IDs “159”, “1”, “7373”, “7371”, and “7374” follow in decreasing order. The counts are plotted on a logarithmic scale.
 
 ![Thread ID](pics/train_threadid_plot.png){width=40%, height=35%}

 - parentProcessId: parent process id (integer)
 Parent process ID “187” has the highest count, nearing 100,000 occurrences. Other IDs such as “7099”, “1”, “1469”, “188”, and “1336” show decreasing counts. The counts are plotted on a logarithmic scale.
...
\newpage
 ...
 ![User ID](pics/train_userid_plot.png){width=30%, height=20%}


 - mountNamespace: Set mounting restrictions this process log (integer) \
 The mount namespace with the label “4026531840” has the highest count, reaching more than 100,000 occurrences. Other labels such as “4026532217”, “4026532232”, “4026532229”, “4026532231”, and “4026532288” follow in descending order.
...
\newpage
 ...
 Other process names such as “systemd-udevd”, “sshd”, “systemd-journal”, and “systemd” follow in decreasing order. A large number of processes are included, some with very small counts compared to “ps”.
    
 ![Process name](pics/train_processname_plot.png){width=43%, height=43%}


 - hostName: host server (string)\
 The hostname “ubuntu” has the highest count, nearly reaching 200,000 occurrences. Other hostnames such as “ip-10-100-1-57”, “ip-10-100-1-120”, “ip-10-100-1-28”, “ip-10-100-1-55”, and “ip-10-100-1-173” show a descending count, with the least count observed for “ip-10-100-1-79”
 
 ![Host name](pics/train_hostname_plot.png){width=20%, height=25%}

\newpage

 - eventId and eventName: id of the event generating this log (integer) \
  Event ID (or Event Name) “3” has the highest count, exceeding (10^5). Following IDs include “257”, “1005”, “4”, “21”, “1008”, and many others, with gradually fewer occurrences.
  
 ![Event ID](pics/train_eventid_plot.png){width=40%, height=50%}
 
 ![Event name](pics/train_eventname_plot.png){width=40%, height=50%}
...
\newpage
 ...
 ![Return value](pics/ReturnValue_barplot.png){width=35%, height=35%}
...
## Args
Args column contains a list of maximum 5 dictionaries, each dictionary contains three elements ({'name': 'dev', 'type': 'dev_t', 'value': 211812353}).
We created 15 new columns in each dataset, and assigns each element from the dictionaries to its respective new column.
...
As our approach is for an unsupervised model, we used ordinal encoder to handle new classes not present in the training dataset.\
...
## Smote
Dealing with unbalanced data can be tricky, most of the machine learning model will give good results for big classes and poor performance on the minority although, as it is our case, minority class is more important.
To balance that, we tried to use Smote library combined as it is adviced with random undersampling for the majority class.
SMOTE (Synthetic Minority Oversampling Technique) [2] works by interpolating new instances along line segments joining existing minority class instances.
...
With the concept of matrix profile, we tried to find conserved behaviors in the data. In fact, a comparison between sequences can be done by looking at the euclidean distance between all the points in two subsequences and represent the distances in a matrix profile.

For the high computational requirements, in order to experiment the shapelet discovery method, we decided to adopt a sample of 230.000 datapoints and see how this method performs for the Decision tree classifier, LSTM and the Dense model.
...
### Model 1
   ...
   - **Description** \
      ...
      ![Dense neural network - Model 1](pics/Dense-model1-structure.png){width=50%, height=50%}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Dense_training.png}
    % \caption{Caption for first image} % optional
  \end{minipage}\\[1ex] % Adjust the spacing between the rows as needed
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Dense_training2.png}
...
### Model 2
   - **Description**
      ...
      ![Dense neural network - Model 2](pics/Dense-model2-structure.png){width=50%, height=60%}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Dense2_training.png}
    % \caption{Caption for first image} % optional
  \end{minipage}\\[1ex] % Adjust the spacing between the rows as needed
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Dense2_training2.png}
    % \caption{Caption for second image} % optional
  \end{minipage}
...
### Model 3
   - **Description**
      This model is similar to **Model 1**, this model is trained on data after applying Smote data augmentation technic.
      
      ![Dense neural network - Model 3](pics/Dense-smote-structure.png){width=50%, height=60%}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Dense_smote_training2.png}
    % \caption{Caption for first image} % optional
  \end{minipage}\\[1ex] % Adjust the spacing between the rows as needed
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Dense_smote_training.png}
    % \caption{Caption for second image} % optional
  \end{minipage}
...
### Model 4
   - **Description** \
      ...
      ![Dense neural network - Model 4](pics/Dense-model4-structure.png){width=50%, height=60%}
...
### Model 5
   - **Description**\
      ...
      ![Conv. neural network - Model 5](pics/Conv-model1-structure.png){width=50%, height=60%}
...
### Model 6
   - **Description**\
      ...
      ![Conv. neural network - Model 6](pics/Conv-model2-structure.png){width=50%, height=60%}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Conv-model2-training2.png}
    % \caption{Caption for first image} % optional
  \end{minipage}\\[1ex] % Adjust the spacing between the rows as needed
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/Conv-model2-training.png}
    % \caption{Caption for second image} % optional
  \end{minipage}
...
### Model 7
   - **Description**\
      ...
      It consists of four LSTM layers, each with 32 units and ReLU activations, using Lecun normal initialization for the kernels and a custom random normal initializer for the biases, followed by dropout layers to prevent overfitting.
       ...
       ![LSTM - Model 7](pics/LSTM-model1-structure.png){width=50%, height=60%}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/LSTM-model1-training.png}
    % \caption{Caption for first image} % optional
  \end{minipage}\\[1ex] % Adjust the spacing between the rows as needed
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/LSTM-model1-training2.png}
    % \caption{Caption for second image} % optional
  \end{minipage}
...
### Model 8
   - **Description** \
      This model integrates various types of inputs through embedding layers to handle categorical data, and dense layers for numerical data.\
      The use of LSTM layers suggests that the model is designed to capture temporal dependencies or sequences in the data.\
      Multiple dense and dropout layers are used to refine the features and prevent overfitting before producing the final classification output.\

      ![LSTM - Model 8](pics/LSTM-model2-structure.png){width=60%, height=60%}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/LSTM-model2-training.png}
    % \caption{Caption for first image} % optional
  \end{minipage}\\[1ex] % Adjust the spacing between the rows as needed
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.55\textwidth]{pics/LSTM-model2-training2.png}
    % \caption{Caption for second image} % optional
  \end{minipage}
...
### Model 9
  ...
  ![LSTM - Model 9](pics/LSTM-model3-structure.png){width=50%, height=50%}

\newpage

   - **Training**\
   
      As the graphs show, the LSTM doesn't give good results when you apply the shapelet discovery method. The performance of the model is really poor and ineffective.
...
## Transformer
  ...
  ![Transformer structure](pics/Transformer-structure.png){width=70%, height=70%}
...
\newpage
  ...
  - Positional encoding adds information about the position of each element in the sequence by creating a positional encoding matrix, which applies sinusoidal functions to encode positional information.\
   ...
   - This model uses Adam optimizer with a **WarmUpCosineDecay** learning rate. During the warm-up phase, the learning rate increases linearly, and once the warm-up steps are completed, it follows a cosine decay pattern to gradually reduce the learning rate.
...
- **Prediction**:
  ...
  The confusion matrix shows a small number of False Positives (25), which is good as it shows the model rarely predicts class 1 when the true class is 0. A relatively low number of False Negatives (1989), indicating the model occasionally misses class 1 predictions. Overall the model seems to learn.
...
* Dense Neural Networks (DNNs): The DNN models showed varying results, with some models performing well and others not. The use of embeddings in DNNs (Model 2) significantly improved performance compared to models without embeddings. However, Model 3, after applying SMOTE, showed insufficient performance, indicating a lack of learning ability from the augmented data. The method of Shapelet Discovery applied on the Dense model shows a good performance on a small sample of data that indicate a potentiality of this method applied to the Dense algorithm.
...
* Decision Tree Classifier with Shapelet Discovery: this approach presents a low number of misclassifications and a good accuracy level.
